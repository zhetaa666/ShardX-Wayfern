...
* **Profile workspace** — per-profile `user-data-dir`, persistent
  ...
  to top, clone.
* **Chromium 148 compatibility default (Windows)** — new profiles use the
  pinned ixBrowser Chromium 148 runtime by default, with ShardX Chromium 149
  and Chromium 145 still available as per-profile alternatives. Existing
  profiles keep their stored engine, and each version has isolated browser
  data. The ShardX fingerprint library is adapted to ixBrowser's native config
  format and automation remains standard CDP. Download a pinned runtime or
  choose a complete local `chrome.exe` bundle in Settings; no third-party
  binary is bundled inside the launcher.
